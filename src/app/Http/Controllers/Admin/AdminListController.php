...
class AdminListController extends Controller
{
    public function index()
    {
        ...
        // 出勤時間の年月日を検索
        $works = Work::where('attendance_time', "LIKE", '%' . substr($now_date, 0, 10) . '%')->get();
        ...
        return view('admin/list', compact('lists', 'now_date'));
    }
    ...
    public function list($works, $rests, $rest_minute)
    {
        for ($i = 0; $i < count($works); $i++) {
            ...
            $work_id = $works[$i]['id'];
            // 名前
            $name = User::find($works[$i]['user_id'])->name;
            // 出勤時間
